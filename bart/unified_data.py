...
class UnifiedQAData(QAData):
    def __init__(self, logger, args, data_path, is_training):
        ...
        self.data_type = data_path.split("/")[-1][
            :-4
        ]  # TJH strip .tsv from filename appearing after final "/"
        ...
        for dataset in self.unified_dataset:
            ...
            curr_data_path = data_path.replace(
                "{}.tsv".format(self.data_type),
            ...
            with open(curr_data_path, "r") as f:
                ...
                for line in f:
                    ...
                    self.data[dataset]["id"].append(
                        "{}-{}-{}".format(dataset, self.data_type, cnt)
                    )
    ...
    def load_dataset(self, tokenizer):
        ...
        if self.load and os.path.exists(preprocessed_path):
            ...
            for dataset in self.unified_dataset:
                metadata.append(
                    (
                        len(questions),
                        len(questions) + len(self.data[dataset]["question"]),
                    )
                )
            ...
            answer_input = self.tokenizer.batch_encode_plus(
                answers,
                truncation=True,  # TJH added
                padding="max_length",  # TJH was pad_to_max_length=True,
                max_length=self.args.max_input_length,
        ...
        self.dataset = MyUnifiedQADataset(
            input_ids,
            attention_mask,
            decoder_input_ids,
            decoder_attention_mask,
    ...
    def evaluate(self, predictions):
        ...
        for i, dataset in enumerate(self.unified_dataset):
            ...
            em = np.mean(
                [
                    get_exact_match(prediction, gt)
                    for (prediction, gt) in zip(
                        _predictions, self.data[dataset]["answer"]
                    )
                ]
            )
            ems.append(em)
...
class MyUnifiedQADataset(Dataset):
    def __init__(
        ...
        self.indices = [
            np.random.permutation(range(start, end)) for start, end in self.metadata
        ]
        self.positions = [0 for _ in self.metadata]
        self.length = (
            len(self.metadata) * np.min([end - start for start, end in self.metadata])
            if is_training
    ...
    def __getitem__(self, idx):
        ...
        return (
            ...
            self.attention_mask[dp_idx],
            self.decoder_input_ids[dp_idx],
            self.decoder_attention_mask[dp_idx],
        )
