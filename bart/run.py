...
def run(args, logger):
    ...
    if args.do_train:
        ...
        if args.checkpoint is not None:
            model = MyBart.from_pretrained(
                "facebook/bart-large", state_dict=torch.load(args.checkpoint)
            )  # TJH: bart-large
            logger.info(
                "Loading checkpoint from {}".format(args.checkpoint)
            )  # TJH Added
        else:
    ...
    if args.do_predict:
        ...
        model = MyBart.from_pretrained(
            "facebook/bart-large", state_dict=torch.load(checkpoint)
        )  # TJH: bart-large
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for batch in train_data.dataloader:
            if args.verbose and global_step % 100 == 0:
                logger.info(
                    "Epoch %d   Global Step %d" % (epoch, global_step)
                )  # TJH Added
            global_step += 1
            ...
            if global_step % args.eval_period == 0:
                if args.skip_inference:
                    logger.info(
                        "Step %d (epoch %d) Train loss %.2f"
                        % (global_step, epoch, np.mean(train_losses))
                    )
                    train_losses = []
                    model_state_dict = {
                    ...
                    if args.verbose:
                        logger.info(
                            "Step %d Starting inference.." % (global_step)
                        )  # TJH Added
                    model.eval()
                    curr_em = inference(
                    ...
                    if best_accuracy < curr_em:
                        ...
                        wait_step += 1
                        logger.info(
                            "No improvement. Number of wait steps: %d of max wait steps: %d"
                            % (wait_step, args.wait_step)
                        )
                        if wait_step >= args.wait_step:
                            stop_training = True
                            logger.info(
                                "Early Stopping due to no improvement after %d wait steps!"
                                % (wait_step)
                            )  # TJH Added
                            break
                model.train()
